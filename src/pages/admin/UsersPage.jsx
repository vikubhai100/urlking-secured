...
export default function UserModal({ user, adminToken, onClose, onSaved }) {
  const [activeTab, setActiveTab] = useState('stats'); // info, stats, payment, finance, config
  const [withdrawals, setWithdrawals] = useState([]);
  const [loadingHistory, setLoadingHistory] = useState(false);

  // 💰 Financial Calculations
  const totalEarnings = parseFloat(user.stats?.earnings || 0);
  const totalWithdrawn = parseFloat(user.stats?.withdrawn || 0); // Backend se aana chahiye
  const totalPending = parseFloat(user.stats?.pending || 0); // Backend se aana chahiye
      ...
      <div className="bg-white w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-bold text-slate-800 leading-tight">@{user.username}</h3>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">User Management Interface</p>
            </div>
          </div>
          ...
          {activeTab === 'stats' && (
            <div className="grid grid-cols-2 gap-4 animate-fadeIn">
               ...
               <div className="p-5 rounded-3xl bg-emerald-50 border border-emerald-100">
                  ...
                  <p className="text-[10px] font-bold text-emerald-500 uppercase">Total Earnings</p>
                  <p className="text-2xl font-black text-emerald-700">{fmt$(totalEarnings)}</p>
               </div>
            </div>
          )}

          {/* 💰 FINANCE TAB (New Section) */}
          {activeTab === 'finance' && (
            <div className="space-y-6 animate-fadeIn">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="p-4 rounded-2xl bg-white border border-slate-100 shadow-sm">
                  <p className="text-[9px] font-bold text-slate-400 uppercase mb-1">Total Withdrawn</p>
                  <p className="text-xl font-black text-emerald-600 font-mono">{fmt$(totalWithdrawn)}</p>
                </div>
                <div className="p-4 rounded-2xl bg-white border border-slate-100 shadow-sm">
                  <p className="text-[9px] font-bold text-slate-400 uppercase mb-1">Pending Amount</p>
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
                  <i className="fas fa-history text-slate-300" /> Withdrawal History
                </h4>
                <div className="border border-slate-100 rounded-2xl overflow-hidden">
                  <table className="w-full text-xs">
                    <thead className="bg-slate-50/50 border-b border-slate-100">
                      <tr>
                        <th className="p-3 text-left font-bold text-slate-400">DATE</th>
                        <th className="p-3 text-center font-bold text-slate-400">AMOUNT</th>
                        <th className="p-3 text-right font-bold text-slate-400">STATUS</th>
                      </tr>
                    </thead>
                      ...
                      {loadingHistory ? (
                        [...Array(3)].map((_, i) => (
                          <tr key={i} className="animate-pulse">
                            <td colSpan="3" className="p-4"><div className="h-4 bg-slate-50 rounded w-full" /></td>
                          </tr>
                        ))
                      ) : withdrawals.length === 0 ? (
                        <tr><td colSpan="3" className="p-10 text-center text-slate-300 italic">No history available</td></tr>
                      ) : withdrawals.map(w => (
                        <tr key={w.id} className="hover:bg-slate-50/50">
                          <td className="p-3 text-slate-500 font-medium">{new Date(w.created_at).toLocaleDateString()}</td>
                          <td className="p-3 text-center font-black text-slate-700 font-mono">{fmt$(w.amount)}</td>
                          <td className="p-3 text-right">
                            <Badge color={w.status === 'approved' ? 'green' : w.status === 'pending' ? 'yellow' : 'red'}>
                              {w.status}
                            </Badge>
                          </td>
                        </tr>
            ...
            <div className="space-y-4 animate-fadeIn">
               <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input readOnly value={user.username} className="w-full mt-1 p-3 rounded-xl bg-slate-50 border border-slate-100 text-slate-800 text-sm font-semibold outline-none" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Role</label>
                    <input readOnly value={user.role || 'User'} className="w-full mt-1 p-3 rounded-xl bg-slate-50 border border-slate-100 text-slate-800 text-sm font-semibold outline-none" />
                  </div>
               </div>
               <div>
                  <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Email Address</label>
                  <input readOnly value={user.email} className="w-full mt-1 p-3 rounded-xl bg-slate-50 border border-slate-100 text-slate-800 text-sm font-semibold outline-none" />
               </div>
            </div>
          )}

          {/* ... (Config & Payment tabs ka logic bhi yahan add kar sakte hain) */}
        </div>

        {/* --- FOOTER --- */}
        <div className="p-5 bg-slate-50/50 border-t border-slate-100 flex justify-end gap-3">
          <button onClick={onClose} className="px-6 py-2.5 text-xs font-bold text-slate-400 hover:text-slate-600 uppercase tracking-wider">Close</button>
          <button onClick={onSaved} className="px-8 py-3 bg-violet-600 hover:bg-violet-700 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-violet-200 transition-all active:scale-95">
            Save Changes
          </button>
        </div>
      </div>
