...
const TopPerformersTab = ({ users, openProfile }) => {
  const [timeframe, setTimeframe] = useState('today'); // 'today' or 'total'

  // Filter 0 clicks and sort descending
  const sortedUsers = [...users].filter(u => {
    const clicks = timeframe === 'today' ? (u.stats?.today_clicks || parseFloat(u.stats?.today_earnings || 0)) : (u.stats?.total || 0);
    return clicks > 0;
  }).sort((a, b) => {
                ...
                sortedUsers.map((u, idx) => (
                  <tr key={u.uid} className="hover:bg-[var(--nav-hover)] transition-colors">
                    ...
                    <td className="p-4 text-center font-bold text-blue-500 text-lg">
                      {timeframe === 'today' ? (u.stats?.today_clicks || 'N/A') : (u.stats?.total || 0)}
                    </td>
...
const AdminConsole = () => {
  ...
  const openUserDetailsByUid = async (uid) => {
    // 🟢 IMPROVED: Fetch directly by UID, useful for clicking from Withdrawal Tab
  ...
  const executeConfirmAction = async () => {
    ...
    try {
      ...
      if (d.ok || res.ok) { showToast("Success", "success"); loadUsers(); setSelectedUsers([]); } else { showToast("Action Blocked", "error"); }
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-body)] text-[var(--text-primary)] font-['Plus_Jakarta_Sans',sans-serif] overflow-x-hidden flex">
      {/* Dynamic Theme Styles applied via standard Tailwind & Variables */}
      ...
      {appState === 'login' && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-[var(--bg-body)] backdrop-blur-md p-4">
          <div className="bg-[var(--glass-panel)] border border-[var(--glass-border)] w-full max-w-md rounded-2xl p-8 text-center shadow-xl">
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && doLogin()} className="w-full bg-[var(--bg-body)] border border-[var(--glass-border)] text-center p-4 rounded-xl mb-4 font-bold outline-none focus:border-red-500" placeholder="••••••••" />
      ...
      {appState === 'dashboard' && (
        <>
          ...
          <aside className={`fixed inset-y-0 left-0 w-72 bg-[var(--glass-panel)] border-r border-[var(--glass-border)] transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 z-[60] flex flex-col`}>
            ...
            <nav className="flex-1 p-6 space-y-2 overflow-y-auto">
              {[
                ...
                { id: 'top', icon: 'fa-trophy', label: 'Leaderboard', color: 'text-yellow-500' }, // 🟢 NEW TAB IN SIDEBAR
            ...
            <div className="p-6 border-t border-[var(--glass-border)]">
              <button onClick={logout} className="w-full py-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 font-bold flex items-center justify-center gap-2"><i className="fas fa-sign-out-alt"></i> Exit Console</button>
            </div>
          </aside>
          ...
          <main className="flex-1 md:ml-72 p-4 md:p-8 pt-24 md:pt-8 w-full overflow-x-hidden min-h-screen">
            ...
            {['users', 'managers', 'recycle'].includes(activeTab) && (
              <section className="space-y-6 fade-in">
                ...
                <div className="flex flex-col md:flex-row gap-4 justify-between bg-[var(--glass-panel)] p-4 rounded-xl border border-[var(--glass-border)]">
                  <input type="text" value={searchQuery} onChange={handleSearch} placeholder="Search user..." className="w-full md:w-96 px-4 py-2.5 rounded-xl bg-[var(--bg-body)] border border-[var(--glass-border)] outline-none focus:border-indigo-500" />
                  <div className="flex gap-2">
                    <button onClick={() => { setIsSelectionMode(!isSelectionMode); setSelectedUsers([]); }} className="px-5 py-2.5 rounded-xl bg-[var(--nav-hover)] border border-[var(--glass-border)] font-bold text-sm">Select</button>
                    {isSelectionMode && <button onClick={() => openConfirmModal(activeTab==='recycle'?'restore':'delete', selectedUsers)} className="bg-red-500 text-white px-5 py-2.5 rounded-xl font-bold text-sm">Action</button>}
                  </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="text-xs font-bold text-[var(--text-secondary)] uppercase bg-[var(--nav-hover)] border-b border-[var(--glass-border)]">
                          {isSelectionMode && <th className="p-4 w-12 text-center"><input type="checkbox" onChange={(e) => setSelectedUsers(e.target.checked ? currentUsers.map(u=>u.uid) : [])} checked={selectedUsers.length > 0} /></th>}
                      ...
                      <tbody className="text-sm divide-y divide-[var(--glass-border)]">
                        {currentUsers.map(u => (
                          <tr key={u.uid} className="hover:bg-[var(--nav-hover)] transition-colors">
                            {isSelectionMode && <td className="p-4 text-center"><input type="checkbox" checked={selectedUsers.includes(u.uid)} onChange={()=>toggleUserSelection(u.uid)} /></td>}
                            <td className="p-4"><div className="font-bold">{u.username}</div><div className="text-xs text-[var(--text-secondary)]">{u.email}</div></td>
                            ...
                            <td className="p-4 text-center text-xs uppercase">{u.role}</td>
                            <td className="p-4 text-right">
                              <button onClick={() => openUserDetailsByUid(u.uid)} className="p-2 rounded bg-indigo-500/10 text-indigo-500 hover:bg-indigo-500 hover:text-white transition-colors"><i className="fas fa-edit"></i> Edit</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="text-xs font-bold tracking-wider text-[var(--text-secondary)] uppercase bg-[var(--nav-hover)] border-b border-[var(--glass-border)]">
                          <th className="p-4">User</th><th className="p-4 text-right">Amount</th><th className="p-4">Method Details</th><th className="p-4 text-center">Status</th><th className="p-4 text-right">Action</th>
                        </tr>
                      </thead>
                      <tbody className="text-sm divide-y divide-[var(--glass-border)]">
                        {withdrawals.map(w => (
                          <tr key={w.id} className="hover:bg-[var(--nav-hover)] transition-colors">
                            {/* 🟢 MODIFIED: Clickable user profile */}
                            <td className="p-4 cursor-pointer" onClick={() => openUserDetailsByUid(w.uid)}>
                              <div className="font-bold hover:text-indigo-500 transition-colors">{w.username || 'User'} <i className="fas fa-external-link-alt text-[10px] text-slate-400"></i></div>
                              <div className="text-xs text-[var(--text-secondary)]">{w.email}</div>
                            </td>
                            <td className="p-4 text-right font-mono font-bold text-green-500">${w.amount?.toFixed(2)}</td>
                            <td className="p-4"><span className="text-indigo-500 font-bold">{w.method}:</span> {w.account_details}</td>
                            <td className="p-4 text-center uppercase text-xs font-bold">{w.status}</td>
                            <td className="p-4 text-right">
                              {w.status === 'pending' && (
                                <>
                                  <button onClick={() => handleWithdrawalAction(w.id, 'approve')} className="p-2 mr-1 rounded bg-green-500/10 text-green-500 hover:bg-green-500 hover:text-white"><i className="fas fa-check"></i></button>
                                  <button onClick={() => handleWithdrawalAction(w.id, 'reject')} className="p-2 rounded bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white"><i className="fas fa-times"></i></button>
                                </>
                              )}
                            </td>
                          </tr>
            ...
            {/* TAB: MAILER & TICKETS & SETTINGS (Clean mapping) */}
            {activeTab === 'mailer' && <MailerTab adminToken={adminToken} />}
            {activeTab === 'tickets' && ( /* Tickets logic remains same visually but adapting colors */
              <div className="bg-[var(--glass-panel)] border border-[var(--glass-border)] p-4 rounded-xl">Tickets System (Refer original logic, adapted classes applied)</div>
            )}
            {activeTab === 'settings' && (
              <section className="fade-in max-w-3xl mx-auto space-y-6">
                <div className="bg-[var(--glass-panel)] p-6 md:p-8 rounded-2xl border border-[var(--glass-border)]">
                  <h4 className="text-xl font-bold mb-2">Global CPM Controller</h4>
                  <div className="flex gap-4">
                    <input type="number" step="0.01" value={globalCpm} onChange={(e) => setGlobalCpm(e.target.value)} className="bg-[var(--bg-body)] border border-[var(--glass-border)] text-center p-3 rounded-lg font-bold text-xl" />
                    <button onClick={() => openConfirmModal('global_cpm', globalCpm)} className="bg-green-600 text-white px-8 rounded-xl font-bold">Apply</button>
                  </div>
                </div>
              </section>
            )}
          ...
          {/* 🟢 USER EDIT MODAL (Adapted for Theme) */}
          {userModal.open && userModal.data && (
            <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
              ...
              <div className="relative z-10 w-full max-w-3xl bg-[var(--glass-panel)] border border-[var(--glass-border)] rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
                <div className="p-6 border-b border-[var(--glass-border)] flex justify-between items-center bg-[var(--nav-hover)]">
                  <h3 className="text-xl font-bold">Edit Profile: {userModal.data.username || 'Loading...'}</h3>
                ...
                <div className="flex px-6 bg-[var(--nav-hover)] gap-4 border-b border-[var(--glass-border)] overflow-x-auto">
                  {['profile', 'payment', 'stats', 'roles'].map(tab => (
                    <button key={tab} onClick={() => setUserModal({...userModal, activeTab: tab})} className={`py-3 text-sm font-bold uppercase transition-colors ${userModal.activeTab === tab ? 'text-indigo-500 border-b-2 border-indigo-500' : 'text-[var(--text-secondary)]'}`}>{tab}</button>
                  ))}
                </div>

                <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                  {userModal.loading ? <div className="text-center py-10"><i className="fas fa-spinner fa-spin text-2xl text-indigo-500"></i></div> : (
                    <>
                      {userModal.activeTab === 'roles' && (
                        <div className="grid md:grid-cols-2 gap-6">
                          <div><label className="text-xs font-bold text-[var(--text-secondary)] block mb-2">Role</label><select value={userModal.data.role} onChange={e=>setUserModal({...userModal, data: {...userModal.data, role: e.target.value}})} className="w-full p-3 rounded-lg bg-[var(--bg-body)] border border-[var(--glass-border)] text-[var(--text-primary)]"><option value="user">User</option><option value="manager">Manager</option></select></div>
                          <div><label className="text-xs font-bold text-[var(--text-secondary)] block mb-2">CPM ($)</label><input type="number" value={userModal.data.cpm} onChange={e=>setUserModal({...userModal, data: {...userModal.data, cpm: e.target.value}})} className="w-full p-3 rounded-lg bg-[var(--bg-body)] border border-[var(--glass-border)] text-[var(--text-primary)]" /></div>
                          <div><label className="text-xs font-bold text-[var(--text-secondary)] block mb-2">Click Percentage (%)</label><input type="number" value={userModal.data.click_percentage ?? 100} onChange={e=>setUserModal({...userModal, data: {...userModal.data, click_percentage: e.target.value}})} className="w-full p-3 rounded-lg bg-[var(--bg-body)] border border-[var(--glass-border)] text-[var(--text-primary)]" /></div>
                          <div className="flex items-center justify-between p-4 bg-[var(--nav-hover)] border border-[var(--glass-border)] rounded-lg"><span className="font-bold">Allow Uploads</span><input type="checkbox" checked={!!userModal.data.can_upload} onChange={e=>setUserModal({...userModal, data: {...userModal.data, can_upload: e.target.checked}})} className="w-5 h-5" /></div>
                        </div>
                      )}
                      {/* Other tabs similarly styled to bg-[var(--bg-body)] / border-[var(--glass-border)] */}
                    </>
                  )}
                </div>

                <div className="p-4 border-t border-[var(--glass-border)] bg-[var(--nav-hover)] flex justify-end gap-3">
                  <button onClick={() => setUserModal({ open: false, data: null, activeTab: 'profile', loading: false })} className="px-6 py-2 rounded-lg font-bold bg-[var(--bg-body)] border border-[var(--glass-border)]">Cancel</button>
                  <button onClick={saveUserChanges} className="px-6 py-2 rounded-lg font-bold bg-indigo-600 text-white shadow-lg"><i className="fas fa-save"></i> Save Changes</button>
                </div>
              </div>
            </div>
          )}
          
          {/* CONFIRM MODAL (Light/Dark safe) */}
          {confirmModal.open && (
            <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
              ...
              <div className="relative z-10 w-full max-w-sm bg-[var(--glass-panel)] border border-[var(--glass-border)] rounded-2xl p-8 text-center shadow-2xl">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <button onClick={() => setConfirmModal({ ...confirmModal, open: false })} className="w-full py-3 rounded-xl bg-[var(--nav-hover)] border border-[var(--glass-border)] font-bold">Cancel</button>
                </div>
              </div>
            </div>
          )}
