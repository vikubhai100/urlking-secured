...
const ManageFiles = ({ token }) => {
  ...
  const loadFiles = async (p) => {
    setLoading(true);
    try {
      // Backend ko page number bheja ja raha hai
      const res = await fetch(`${API}/api/dev/my-files?page=${p}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  // SAFETY LOCK: Agar backend saari files ek sath bhej de, toh usey kaat kar sirf 5 dikhayenge
        ...
        <div className="overflow-x-auto flex-1 p-2">
          <table className="w-full text-left border-collapse min-w-[600px]">
            ...
            <tbody className="divide-y divide-[var(--glass-border)] text-sm text-[var(--text-primary)]">
              {loading ? (
                <tr><td colSpan="5" className="p-6 text-center text-slate-500"><i className="fas fa-circle-notch fa-spin text-2xl mb-2 text-indigo-500"></i><br/>Loading...</td></tr>
                  ...
                  return (
                    <tr key={f.short_id} className="hover:bg-[var(--nav-hover)] transition-colors">
                      ...
                      <td className="p-4 pr-6 text-right">
                        ...
                        <button onClick={() => copyLink(shortUrl)} className="p-2 rounded-lg bg-green-500/10 hover:bg-green-600 text-green-500 hover:text-white transition-all text-xs"><i className="fas fa-copy"></i></button>
                      </td>
                    </tr>
                  );
                })
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="glass-panel p-6 rounded-2xl w-full max-w-md">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-[var(--glass-border)]">
              <button onClick={() => setRenameModal({ open: false, id: '', name: '' })} className="px-4 py-2 text-slate-400 hover:bg-[var(--nav-hover)] rounded">Cancel</button>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="glass-panel p-6 rounded-2xl w-full max-w-sm text-center">
            ...
            <div className="flex justify-center gap-3">
              <button onClick={() => setDeleteModal({ open: false, id: '' })} className="px-4 py-2 text-slate-400 hover:bg-[var(--nav-hover)] rounded">Cancel</button>
              <button onClick={handleDelete} className="px-6 py-2 bg-red-600 hover:bg-red-700 text-white rounded font-bold">Yes, Delete</button>
            </div>
          </div>
        </div>
      )}
