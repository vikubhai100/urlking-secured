...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans">
      ...
      {sidebar && (
        <div 
          className="fixed inset-0 z-30 bg-black/30 md:hidden backdrop-blur-sm" 
      ...
      >
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto custom-scrollbar">
          {NAV.map(n => (
            <NavLink 
              ...
              onClick={() => setSidebar(false)} // Mobile me click ke baad close karne ke liye
              className={({ isActive }) => `
                w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all
                ${isActive 
                  ? 'bg-violet-50 text-violet-700 font-bold shadow-sm border border-violet-100/50' 
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50">
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl text-sm text-red-600 bg-red-50 hover:bg-red-100 border border-red-100 font-bold transition-colors"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 p-4 md:p-6 max-w-7xl w-full mx-auto animate-fadeIn">
          {/* <Outlet /> is where the child routes (Overview, Users, etc.) will render */}
          <Outlet /> 
        </main>
      </div>
