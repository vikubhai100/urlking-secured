...
import { showToast } from '../../../toast'; // ✅ Fixed relative path
...
export default function UserModal({ user, allUsers, adminToken, onClose, onSaved, apiFetch }) {
  ...
  // 🚀 Fetch Finance History (Silent & Filtered)
  useEffect(() => {
    if (tab === 'finance') {
      const getHistory = async () => {
        setLoadingHistory(true);
        try {
          // Backend se saari requests mangao
          const res = await apiFetch(`${API}/api/withdraw/admin/requests`, {
            headers: { 'x-admin-token': adminToken }
          });
          // Frontend pe sirf is user (data.uid) ka data filter karo
  ...
  const save = async () => {
    setSaving(true);
    try {
      // 🟢 API Payload matches your db.js columns
    ...
    >
      ...
      {tab === 'info' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-fadeIn">
          {[['Email', data.email, true], ['Username', data.username, true], ['Mobile', data.mobile, true], ['Joined', data.created_at ? new Date(data.created_at).toLocaleDateString() : 'N/A', true]].map(([lbl, val, ro]) => (
            <div key={lbl}>
              ...
              <input readOnly={ro} value={val || ''}
                className="w-full px-3 py-2.5 rounded-xl text-sm bg-slate-50 border border-slate-200 text-slate-700 outline-none" />
            </div>
          ))}
          <div>
          ...
          {/* ✅ Social Links Fixed (Using direct columns from DB) */}
          <div className="sm:col-span-2 space-y-4">
            <div>
              <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Telegram Link</label>
              <div className="flex gap-2">
                <input value={data.telegram || ''} onChange={e => set('telegram', e.target.value)} placeholder="https://t.me/..." className="flex-1 px-3 py-2.5 rounded-xl text-sm bg-white border border-slate-200 text-slate-800 outline-none focus:border-violet-400" />
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input value={data.youtube || ''} onChange={e => set('youtube', e.target.value)} placeholder="https://youtube.com/..." className="flex-1 px-3 py-2.5 rounded-xl text-sm bg-white border border-slate-200 text-slate-800 outline-none focus:border-violet-400" />
        ...
        <div className="space-y-4 animate-fadeIn">
          ...
          <div className="grid grid-cols-2 gap-3">
             <div>
                ...
                <input readOnly value={data.upi_id || 'N/A'} className="w-full px-3 py-2.5 rounded-xl text-sm bg-slate-50 border border-slate-200 font-mono" />
             </div>
            ...
            <table className="w-full text-xs text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {loadingHistory ? <tr><td colSpan="3" className="p-10 text-center animate-pulse text-slate-300">Loading...</td></tr> :
                 history.length === 0 ? <tr><td colSpan="3" className="p-10 text-center text-slate-300 italic">No history found for this user</td></tr> :
                 history.map(w => (
                  <tr key={w.id} className="hover:bg-slate-50">
                    ...
                    <td className="p-3 text-right">
                       <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase ${w.status === 'approved' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>{w.status}</span>
                    </td>
                  </tr>
                 ))}
              </tbody>
        ...
        <div className="space-y-5 animate-fadeIn">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <select value={data.role} onChange={e => set('role', e.target.value)} className="w-full px-3 py-2.5 rounded-xl bg-white border border-slate-200 text-sm">
            ...
            <div>
              ...
              <input type="number" step="0.01" value={data.cpm || ''} onChange={e => set('cpm', e.target.value)} className="w-full px-3 py-2.5 rounded-xl bg-white border border-slate-200 text-sm font-mono" />
            </div>
            <div>
              <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Click %</label>
              <input type="number" value={data.click_percentage ?? 100} onChange={e => set('click_percentage', e.target.value)} className="w-full px-3 py-2.5 rounded-xl bg-white border border-slate-200 text-sm font-mono" />
            </div>
          </div>
