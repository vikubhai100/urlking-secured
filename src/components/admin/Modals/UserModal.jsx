...
import { showToast } from '../../../toast'; // ✅ 3 level peeche src/toast.js tak

const API = import.meta.env.VITE_API_URL || "https://go.urlking.site";

// ─────────────────────────────────────────────
// FETCH HELPER — Aapke working code se copy kiya
// ─────────────────────────────────────────────
...
export default function UserModal({ user, allUsers, adminToken, onClose, onSaved }) {
  ...
  // 🚀 Fetch Finance History (Sirf is user ka)
  useEffect(() => {
    if (tab === 'finance') {
      const getHistory = async () => {
        ...
        try {
          ...
          if (Array.isArray(json)) {
            // 🎯 Main Filter: Sirf user uid match wala data
  ...
  return (
    ...
    >
      ...
      {/* ℹ️ INFO TAB (Restore Exactly as old code) */}
      {tab === 'info' && (
      ...
      {/* 💳 PAYMENT TAB (Restore Exactly as old code) */}
      {tab === 'payment' && (
              ...
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                  {loadingHistory && history.length === 0 ? (
                    <tr><td colSpan="3" className="p-10 text-center italic text-slate-300">Syncing...</td></tr>
                  ) : history.length === 0 ? (
      ...
      {/* 📊 STATS TAB (Restore Exactly as old code) */}
      {tab === 'stats' && (
      ...
      {/* ⚙️ CONFIG TAB (Restore Exactly as old code) */}
      {tab === 'config' && (
